...
@section('content')
    ...
    <div class="main">
        ...
        <div class="sideleft">
            ...
            <div class="news_show">
                <div class="n_s_title"><p>{{$product->title}}</p></div>
                <div class="n_s_xinxi"><p>发布时间：{{ date('Y-m-d',strtotime($product->created_at))}} 点击次数：<span class="ztcolor">{{$product->hit}}</span></p></div>
                <div class="n_s_text">
                    ...
                    <p style="text-align:center;">
                        <img src="{{URL::asset('/')}}backend/{{$product->img_url}}" alt="">
                    </p>
                    <p style="text-align:center;">
                        <br>
                    </p>
                ...
                <div class="switch">
                    @if($pre)
                        <p><span>上一条：</span><a href="{{URL::asset('/')}}product/show?id={{$pre->id}}">{{$pre->title}}</a></p>
                    @endif
                    @if($next)
                        <p><span>下一条：</span><a href="{{URL::asset('/')}}product/show?id={{$next->id}}">{{$next->title}}</a></p>
                    @endif
                </div>
